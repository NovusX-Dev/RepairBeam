...
Example: "FakeModel123" -> {"isValid": false, "correctedName": null, "confidence": 0.0}`;
  ...
  // Generate contextual repair tips
  async generateRepairTips(deviceType: string, deviceModel: string, issueDescription: string, ticketStatus: string): Promise<{
    tips: Array<{
      category: string;
      title: string;
      description: string;
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        console.log(`🔧 Generating repair tips for ${deviceType} ${deviceModel} (${ticketStatus})`);
        
        const prompt = `You are an expert repair technician with decades of experience fixing ${deviceType} devices. 
        
Provide practical repair tips and diagnostic steps for this scenario:
- Device: ${deviceType} ${deviceModel}
- Issue: ${issueDescription || 'General maintenance and repair'}
...
Return your response as JSON with this exact structure:
{
  "tips": [
    {
      "category": "diagnosis" | "repair" | "prevention" | "tools",
      "title": "Brief, actionable tip title",
...
Focus on:
1. Device-specific repair techniques for ${deviceModel}
2. Common issues and their solutions
3. Safety warnings and precautions
4. Professional repair tips
5. Time and difficulty estimates

Provide 3-5 practical tips relevant to the current ticket status: ${ticketStatus}.`;
        ...
        const response = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are a professional repair technician providing expert advice. Always return valid JSON with practical, actionable repair tips."
            },
            {
              role: "user",
              content: prompt
        ...
        if (!result.tips || !Array.isArray(result.tips)) {
          throw new Error('Invalid response structure: missing tips array');
        }

        // Ensure confidence is between 0 and 1
        result.confidence = Math.max(0, Math.min(1, result.confidence || 0.8));

        console.log(`✅ Generated ${result.tips.length} repair tips for ${deviceType} ${deviceModel}`);
        return result;

      } catch (error: any) {
        console.error(`Attempt ${attempt + 1} failed for repair tips generation:`, error);
        
        if (attempt === maxRetries - 1) {
          // Return fallback tips on final failure
          return {
            tips: [
              {
                category: "diagnosis",
                title: "Visual Inspection",
                description: `Perform a thorough visual inspection of the ${deviceType}. Check for physical damage, loose connections, or visible component issues.`,
              ...
              },
              {
                ...
                description: `Gather the appropriate tools for ${deviceType} repair: precision screwdrivers, spudgers, and proper grounding equipment.`,
                difficulty: "easy" as const,
    ...
    throw new Error('Failed to generate repair tips after all retries');
  }
}
